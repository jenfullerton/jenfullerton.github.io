...
var basicSDFFragShader = `
	...
	// wiggleOrbit -- calculates a circular orbit that "wiggles"
	//		in and out from the central point
	//	- calls circularOrbit
	// used alongside TRANSLATE to make shapes follow the path
	// 	VARS:
	// 		focalPt	- point around which to orbit
	// 		r       - radius of orbit
	// 		speed 	- orbit speed; large number = slower, smaller = faster
	//		freq	- wiggle speed
	//		offset	- starting position on unit circle for rotation (in radians)
	vec2 wiggleOrbit(vec2 focalPt, float r, float speed, float offset, float freq, float amp){
		r += sin(time/freq)*amp;
		return circularOrbit(focalPt, r, speed, offset);
	}
	...
	//* hexPrism *//
	float hexPrismSDF(vec3 point, vec2 h){
		vec3 q = abs(point);
	...
	float sceneSDF(vec3 point)
	{
		...
		vec2 orbitA = circularOrbit( center, 0.75, speed, 0.0);
		vec3 centerA = translate(point, orbitA);
		float sphereA = sphereSDF(centerA, 0.20);

		vec2 orbitB = circularOrbit( center, 0.76, speed, -0.3);
		vec3 centerB = translate(point, orbitB);
		float sphereB = sphereSDF(centerB, 0.18);

		vec2 orbitC = circularOrbit( center, 0.77, speed, -0.6);
		vec3 centerC = translate(point, orbitC);
		float sphereC = sphereSDF(centerC, 0.15);

		vec2 orbitD = circularOrbit( center, 0.78, speed, -0.8);
		vec3 centerD = translate(point, orbitD);
		float sphereD = sphereSDF(centerD, 0.12);

		vec2 orbitE = circularOrbit( center, 0.795, speed, -1.0);
		vec3 centerE = translate(point, orbitE);
		float sphereE = sphereSDF(centerE, 0.09);

		vec2 orbitF = circularOrbit( center, 0.81, speed, -1.15);
		vec3 centerF = translate(point, orbitF);
		float sphereF = sphereSDF(centerF, 0.06);

		vec2 orbitG = circularOrbit( center, 0.825, speed, -1.25);
		vec3 centerG = translate(point, orbitG);
		float sphereG = sphereSDF(centerG, 0.03);

		// shape: thing in center to carve
		vec3 hexPtA = rotate(point, 3.14/2.0, 0.0, 3.14/2.0);
		// d, l
		float hexPrismA = hexPrismSDF(hexPtA, vec2(0.2, 1.0));

		vec3 hexPtB = rotate(point, 0.0, 3.14/2.0, 3.14/2.0);
		float hexPrismB = hexPrismSDF(hexPtB, vec2(0.2, 1.0));
		
		// things that goes in an orbit
		float finalShape = blend(sphereA, sphereB, 0.15);
		finalShape = blend(finalShape, sphereC, 0.15);
		finalShape = blend(finalShape, sphereD, 0.11);
		finalShape = blend(finalShape, sphereE, 0.12);
		finalShape = blend(finalShape, sphereF, 0.1);
		finalShape = blend(finalShape, sphereG, 0.1);

		// carve-shape
		float hex = merge(hexPrismA, hexPrismB);
		//hex = merge(hex, hexPrismC);
		//hex = merge(hex, hexPrismD);

		finalShape = intersect(hex, finalShape);
	...
	void main()
	{
		...
		if(dist > END - EPISILON)
		{
			...
			float tColorR = (sin(time/1000.0) + 1.0)/2.0;
			float tColorG = (cos(time/1000.0) + 1.0)/2.0;
			float tColorB = tColorR/2.0 + tColorG/2.0;

			vec3 ka, kd, ks, point;
			ka = vec3(0.0, 0.0, tColorB/2.0);
			kd = vec3(tColorR, tColorG, 0.0);
			ks = vec3(tColorB/2.0, tColorB/2.0, tColorB);
			point = eye+dist*dir;
